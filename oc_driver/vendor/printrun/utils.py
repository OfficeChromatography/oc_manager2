...
def install_locale(domain):
    if os.path.exists('/usr/share/pronterface/locale'):
        ...
        gettext.install(
            domain, '/usr/local/share/pronterface/locale', unicode=1)
    else:
...
def pixmapfile(filename):
    return lookup_file(filename,
                       ['/usr/local/share/pixmaps', '/usr/share/pixmaps'])


def sharedfile(filename):
    return lookup_file(
        filename, ['/usr/local/share/pronterface', '/usr/share/pronterface'])


def configfile(filename):
    return lookup_file(filename, [
        os.path.expanduser("~/.printrun/"),
    ])
...
class RemainingTimeEstimator(object):
    ...
    def __init__(self, gcode):
        ...
        self.gcode = gcode
        self.remaining_layers_estimate = sum(
            layer.duration for layer in gcode.all_layers)
        if len(gcode) > 0:
...
def parse_build_dimensions(bdim):
    ...
    bdl = filter(None, bdl)
    bdl_float = [
        float(value) if value else defaults[i] for i, value in enumerate(bdl)
    ]
    if len(bdl_float) < len(defaults):
        bdl_float += [
            defaults[i] for i in range(len(bdl_float), len(defaults))
        ]
...
def hexcolor_to_float(color, components):
    ...
    div = 16**ndigits - 1
    return tuple(
        round(float(int(color[i:i + ndigits], 16)) / div, 2)
