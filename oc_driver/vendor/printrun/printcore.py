...
def is_something_like_ok(line):
    """ use with caution """
    return line.startswith('ok') or line.startswith('kK')
...
class printcore():
        ...
        if self.port is not None and self.baud is not None:
            ...
            if not is_serial:
                ...
                try:
                    ...
                    self.logError(("Could not connect to %s at baudrate %s:") %
                                  (self.port, self.baud) + "\n" +
                                  _("Serial error: %s") % e)
                    self.printer = None
                    return
        ...
        while not self.online and self.listen_can_continue():
            ...
            while self.listen_can_continue():
                ...
                if line.startswith(tuple(self.greetings)) \
                   or is_something_like_ok(line) or "T:" in line:
                    self.online = True
                    for handler in self.event_handler:
                        try:
                            handler.on_online()
    ...
    def listen(self):
        ...
        while self.listen_can_continue():
            ...
            if line.startswith(tuple(
                    self.greetings)) or is_something_like_ok(line):
                self.clear = True
            if is_something_like_ok(line) and "T:" in line:
                for handler in self.event_handler:
                    try:
                        handler.on_temp(line)
                    except:
                        logging.error(traceback.format_exc())
            if is_something_like_ok(line) and "T:" in line and self.tempcb:
    ...
    def startprint(self, gcode, startindex=0):
        ...
        self.print_thread = threading.Thread(
            target=self.Print, kwargs={"resuming": resuming})
    ...
    def resume(self):
        ...
        if self.paused:
            ...
            self.send_now(
                "G1 X%s Y%s%s" % (self.pauseX, self.pauseY, xyFeedString))
        ...
        self.printing = True
        self.print_thread = threading.Thread(
            target=self.Print, kwargs={"resuming": True})
        self.print_thread.start()
